...
$stmtAnzahl = $db->prepare('SELECT count(*) AS `anzahl` FROM `entries_neu` ');
$stmtAnzahl -> execute();
...
<!--##############################################-->
<?php
$stmt = $db->prepare('SELECT *, entries_neu.created_at AS datum FROM `entries_neu` INNER JOIN `users`
																ON
...
<div class="card p-0 my-2 col-md-12 bg-primary bg-opacity-75 bg-light">
	<div class="card-header">
		<div class="row">
			...
			<span class="col-md-6 name text-end"><?= e($post['nachname'])?></span>
		</div>
	</div>
	<div class="card-body">
		<?php
	...
	<div class="card-footer">
		<em>Geschrieben am: <?= formatiereDatum($post['datum'])?></em>
	</div>
</div>
<?php endforeach;?>
<!--##############################################-->
